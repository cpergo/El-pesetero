...
class TransactionRepositoryImpl(
    ...
    override fun observeMonthlyTotalsRange(from: YearMonth, to: YearMonth): Flow<List<MonthlyTotals>> =
        transactionDao.observeInRange(from.startEpochDay(), to.endEpochDay()).map { transactions ->
            ...
            val grouped = transactions.groupBy { yearMonthOf(localDateFromEpochDay(it.dateEpochDay)) }
            months.map { month ->
    ...
    override fun observeCategoryEvolution(
        ...
        combine(
            ...
            val months = monthsBetween(yearMonthOf(from), yearMonthOf(to))
            val grouped = transactions
                .filter { it.categoryId == categoryId }
                .groupBy { yearMonthOf(localDateFromEpochDay(it.dateEpochDay)) }
            months.map { month ->
